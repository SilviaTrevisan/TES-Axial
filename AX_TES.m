...
dp=0.02; %[m] praticle diameter
void_dp_D = 0.375+0.17*(dp/D)+0.39*((dp/D)^2); %void fraction as f(dp/D)
% void=void_dp_D;
rho_s=2940; %[kg/m3]
c_s=1040; %[J/kg*K]
K_s=2.5; %[W/m*K]
...
alpha_p=(700/(6*(1-void)))*(G^0.76)*(dp^0.24); %[W/K m2] heat transfer coeff air/bed
h_v=(6*(1-void)*alpha_p)/(dp); %[W/K m3] volumetric heat transfer coeff
...
eps_rock = 0.85; rock = 1;
[K_eff_f_avg,K_eff_s_avg] =  Kunii_Smith (Tavg, rock, eps_rock, dp, void);

%for Ansys input
permeability=(dp^2/150)*(void^3/((1-void)^2));
C2=(3.5/dp)*((1-void)/void^3);  %inertial_loss_coeff
...
sol = pdepe(m,@pde_charge_cycles,@ic_charge,@bc_charge_cycles,xmesh,tstep);
...
xlabel('x'), ylabel('Tf'), title('CHARGING'), grid on
...
coeff_dp=1.75*((1-void)/(void^3))+150*(((1-void)^2)/(void^3))*((vi)/(G*dp));
delta_pressure=((H*G^2)/(rho_a*dp))*coeff_dp; %[Pa] pressure drop along the packed bed

%% PDE solution____DISCHARGING______

sol_disch = pdepe(m,@pde_discharging_cycles,@ic_discharging_cycles,@bc_discharging_cycles,xmesh,tstep);
...
xlabel('x'), ylabel('Tf'), title('DISCHARGING'), grid on
...
for n=1:9
    sol = pdepe(m,@pde_charge_cycles,@ic_discharging_cycles,@bc_charge_cycles,xmesh,tstep);
